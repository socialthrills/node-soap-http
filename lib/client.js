...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  http.request(location, xml, function(err, response, body) {
    ...
    if (logging) {
      console.log("Unparsed response:: " + body);
    }
    if (err) {
      callback(err);
    } else {
      ...
      if (logging) {
        console.log('Output 1: ' + JSON.stringify(obj) + " and output name is " + output.$name);
      }
      var name = output.$name;
      if (name.indexOf('1') == name.length-1) {
        name = name.substring(0,name.length-1);
      }
      result = obj.Body[name];
      if (logging) {
        console.log('Output 2: ' + JSON.stringify(result));
      }
      ...
      if (logging) {
        console.log('Output 3: ' + JSON.stringify(result));
      }
